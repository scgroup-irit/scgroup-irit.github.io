...
window.formatGoogleCalendar = function () {
    ...
    var transformationList = function transformationList(result, tagName, format) {
        ...
        var dateFormatted = getFormattedDate(dateStart, dateEnd, dayNames, moreDaysEvent, isAllDayEvent),
            myhour = getFormattedTime(dateStart),
            output = '<' + tagName + '>',
            summary = result.summary || '',
            description = result.description || 'TBA',
        ...
        for (i = 0; i < format.length; i++) {
            ...
            if (format[i] === '*summary*') {
                var temp = "new_window('" + summary +"','" + descriptionString +"','" + dateFormatted + "','" + myhour + "','" + location +"')";
                output = output.concat('<span class="summary"><a href="#" onClick="return ' + temp + ';"> ' + summary + '</a></span>');
                
            } else if (format[i] === '*date*') {
                output = output.concat('<span class="date">' + dateFormatted +  '</span>');
            //} else if (format[i] === '*description*') {                        
            //            output = output.concat('<span class="description">' + descriptionString + '</span>');
            } else if (format[i] === '*location*') {
                output = output.concat('<span class="location">' + location + '</span>');
            } else {
                if (format[i + 1] === '*location*' && location !== '' || format[i + 1] === '*summary*' && summary !== '' || format[i + 1] === '*date*' && 
                dateFormatted !== '' || format[i + 1] === '*description*' && description !== '') {
    ...
    var formatDateSameDay = function formatDateSameDay(dateStart, dateEnd, dayNames, moreDaysEvent, isAllDayEvent) {
        ...
        //return dayNameStart + getMonthName(dateStart[1]) + ' ' + dateStart[0] + ', ' + dateStart[2];
         return getMonthName(dateStart[1])  + ' ' + dayNameStart + ' ' + dateStart[0] + ', ' + dateStart[2] + ' - ' + getFormattedTime(dateStart);
        //return dateStart[2] + ', ' + getMonthName(dateStart[1]) + ' ' +  dateStart[0] +  ' ' + dayNameStart;
    };
    ...
    var formatDateDifferentDay = function formatDateDifferentDay(dateStart, dateEnd, dayNames) {
        ...
        return getMonthName(dateStart[1])+ ' ' + dayNameStart + ' ' + dateStart[0]  + '-' + dayNameEnd + dateEnd[0] + ', ' + dateStart[2];
    };
    ...
    var formatDateDifferentYear = function formatDateDifferentYear(dateStart, dateEnd, dayNames) {
        ...
        return dayNameStart + getMonthName(dateStart[1]) + ' ' + dateStart[0] + ', ' + dateStart[2] + '-' + dayNameEnd + getMonthName(dateEnd[1]) + ' ' + 
        dateEnd[0] + ', ' + dateEnd[2];
    };
    ...
    return {
        init: function init(settingsOverride) {
            var settings = {
                calendarUrl: 'https://www.googleapis.com/calendar/v3/calendars/scgroup.irit@gmail.com/events?key=AIzaSyBXx-_AD-kSg43u3yo2a5jogi6tZa5hZpc',
                past: true,
                upcoming: true,
                sameDayTimes: true,
                dayNames: true,
